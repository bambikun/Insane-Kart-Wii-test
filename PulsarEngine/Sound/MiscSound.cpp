...
static void DisableRaceMusic(Audio::SinglePlayer& singlePlayer, u32 soundId, s16 delay) {
    const bool isEnabled = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_SOUND1, SETTINGS_MENU_MUSIC) == MENUSETTING_MUSIC_DEFAULT;
...
static void DisableMenuMusic(Audio::SinglePlayer& singlePlayer, u32 soundId, s16 delay) {
    const bool isEnabled = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_SOUND1, SETTINGS_MENU_MUSIC) != MENUSETTING_MUSIC_DISABLE_ALL;
    if(isEnabled) singlePlayer.PlaySound(soundId, delay);
}
